 ...
 *
 * Copyright (c) 2009 Matteo Spinelli, http://cubiq.org/ Released under MIT
...
var SpinningWheel = {
    ...
    create: function () {
        ...
        div.id = 'sw-wrapper';
        div.style.top = window.innerHeight + window.pageYOffset + 'px'; // Place
                                                                        // the
                                                                        // SW
                                                                        // down
                                                                        // the
        ...
        div.innerHTML = '<div id="sw-header"><div id="sw-cancel">Cancel</' + 'div><div id="sw-done">Done</' + 'div></'
                + 'div><div id="sw-slots-wrapper"><div id="sw-slots"></' + 'div></' + 'div><div id="sw-frame"></'
                + 'div>';

        document.body.appendChild(div);

        this.swWrapper = div; // The SW wrapper
        this.swSlotWrapper = document.getElementById('sw-slots-wrapper'); // Slots
                                                                            // visible
                                                                            // area
        this.swSlots = document.getElementById('sw-slots'); // Pseudo table
                                                            // element (inner
                                                            // wrapper)
        this.swFrame = document.getElementById('sw-frame'); // The scrolling
                                                            // controller
        ...
        for (l = 0; l < this.slotData.length; l += 1) {
            ...
            div.className = this.slotData[l].style; // Add styles to the
                                                    // container
            div.appendChild(ul);
            ...
            ul.style.webkitTransitionTimingFunction = 'cubic-bezier(0, 0, 0.2, 1)'; // Add
                                                                                    // default
                                                                                    // transition
        ...
        // Global events
        document.addEventListener('touchstart', this, false); // Prevent page
                                                                // scrolling
        document.addEventListener('touchmove', this, false); // Prevent page
                                                                // scrolling
        window.addEventListener('orientationchange', this, true); // Optimize
                                                                    // SW on
                                                                    // orientation
                                                                    // change
        window.addEventListener('scroll', this, true); // Reposition SW on page
                                                        // scroll
    ...
    scrollStart: function (e) {
        ...
        var xPos = e.targetTouches[0].clientX - this.swSlots.offsetLeft; // Clicked
                                                                            // position
                                                                            // minus
                                                                            // left
                                                                            // offset
        ...
        this.slotEl[this.activeSlot].removeEventListener('webkitTransitionEnd', this, false); // Remove
                                                                                                // transition
                                                                                                // event
                                                                                                // (if
                                                                                                // any)
        this.slotEl[this.activeSlot].style.webkitTransitionDuration = '0'; // Remove
                                                                            // any
                                                                            // residual
                                                                            // transition
    ...
    scrollMove: function (e) {
        ...
        if (this.slotEl[this.activeSlot].slotYPosition > 0
                || this.slotEl[this.activeSlot].slotYPosition < this.slotEl[this.activeSlot].slotMaxScroll) {
            topDelta /= 2;
        }
    ...
    if (this.slotEl[this.activeSlot].slotYPosition > 0
            || this.slotEl[this.activeSlot].slotYPosition < this.slotEl[this.activeSlot].slotMaxScroll) {
        this.scrollTo(this.activeSlot, this.slotEl[this.activeSlot].slotYPosition > 0 ? 0
    ...
    if (scrollDistance < this.cellHeight / 1.5 && scrollDistance > -this.cellHeight / 1.5) {
        if (this.slotEl[this.activeSlot].slotYPosition % this.cellHeight) {
            this.scrollTo(this.activeSlot, Math.round(this.slotEl[this.activeSlot].slotYPosition / this.cellHeight)
    ...
    if (newPosition > 0) {
        ...
        // Prevent the slot to be dragged outside the visible area (bottom
        // margin)
        newPosition = (newPosition - this.slotEl[this.activeSlot].slotMaxScroll) / 2
                + this.slotEl[this.activeSlot].slotMaxScroll;
        newDuration /= 3;
...
scrollTo: function (slotNum, dest, runtime) {
    ...
    if (this.slotEl[slotNum].slotYPosition > 0
            || this.slotEl[slotNum].slotYPosition < this.slotEl[slotNum].slotMaxScroll) {
